...
const NewArrival = (props: Props) => {
  return (
    <div className="px-12 lg:px-20 mb-8">
      <h4 className="flex w-full items-center gap-1 lg:gap-2 mb-2">
        ...
        {NewArrivals.map((newBook) => (
          <div key={newBook.id}>
            <Image
              ...
              width={500}
              height={500}
              className="flex w-full h-full shadow-md shadow-slate-500 rounded-sm cursor-pointer"
              //!need to fix the image width & height issues
            />
            <p className="mt-2">{newBook.title}</p>
          </div>
        ))}
      </div>
    </div>
